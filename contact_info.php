...
<head>
	...
	<title>Form 2 - Contact information</title>
...
<body>
		...
		<div class="container">
			<div class="row">
				...
				<!-- Form start -->
				<form class="row mt-3 mb-3 border p-3 bg-light rounded" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" id="contact_info_form" name="contact_info_form" method="post">
					...
					<div class="col-md-6 mt-2">
						...
						<input type="text" class="form-control form-control-lg mt-2" id="email" name="email" value="<?php echo (!empty($fetch_data['email'])) ? $fetch_data['email'] : null; ?>">
						<span id="email_error" class="text-danger p-1"></span>
					</div>
					<!-- Email field end -->
					...
					<div class="col-md-6 row">
						...
						<span title="Phone number should be 10 digits"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#787a7b" class="bi bi-question-circle-fill" viewBox="0 0 16 16">
						...
						<svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" class="bi bi-star-fill star" fill="#c16e7c" viewBox="0 0 16 16">
						 ...
						 :</label>
						<div class="col-3">
							<select class="form-select form-select-lg mt-2" id="country_code" name="country_code">
								<option value=""> -- </option>
								<?php
									$selected_country_code = (!empty($fetch_data['country_code'])) ? $fetch_data['country_code'] : null;
									foreach($country_code as $country_code_key => $country_code_value){
										($selected_country_code == $country_code_key) ? $selected = 'selected' : $selected = '';
										echo '<option value="'.$country_code_key.'"'.$selected.'>'.$country_code_value.'</option>';
									}
								?>
							</select>
						</div>
						...
						<div class="col-9">
							<input type="text" class="form-control form-control-lg mt-2" id="phone" name="phone" value="<?php echo (!empty($fetch_data['phone'])) ? $fetch_data['phone'] : null; ?>">
						</div>
						<span class="d-flex">
							<span id="country_code_error" class="text-danger"></span>
							<span id="phone_error" class="text-danger ms-3"></span>
						</span>
					</div>
					<!-- Phone field end -->
					...
					<div class="col-md-6 mt-2">
						...
						<select id="country" name="country" class="form-select form-select-lg mt-2">
							<option value=""> -- Select Country -- </option>
							<?php
								$selected_country = (!empty($fetch_data['country'])) ? $fetch_data['country'] : null;
						...
						<span id="country_error" class="text-danger p-1"></span>
					</div>
					<!-- Country field end -->
					...
					<div class="col-md-3">
						<a class="btn p-3 previous-btn text-light" href="<?php echo $base_url . '/personal_info.php?id='.$user_id.'&official_info_edit_status='.$official_info_edit_status.'&payment_info_edit_status='.$payment_info_edit_status.'&address_info_edit_status='.$address_info_edit_status; ?>">Previous</a>
						<input type="submit" class="btn p-3 next-btn text-light" id="submit1" name="submit1" value="Next">
					</div>
					<!-- Button field end -->

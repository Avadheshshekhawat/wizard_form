...
<head>
	...
	<title>Form 3 - Official information</title>
...
<body>
		...
		<div class="container">
			<div class="row">
				...
				<!-- Form start -->
				<form class="row mt-3 mb-3 border p-3 bg-light rounded" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" id="official_info_form" name="official_info_form" method="post">
					...
					<div class="col-6 fs-4 text-danger">
						<span class="ms-2">Official information</span>
					</div>
					<div class="col-6 fs-4 text-end">
						<span class="me-3">Step 3/6</span>
					</div>
					
					<!-- Employee ID field start -->
					<div class="col-md-6 mt-2">
						<label for="employee_id">Employee ID
							<span title="Alphanumeric values allowed and length should be 10 characters"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#787a7b" class="bi bi-question-circle-fill" viewBox="0 0 16 16">
						...
						<input type="text" class="form-control form-control-lg mt-2" id="employee_id" name="employee_id" value="<?php echo (!empty($fetch_data['employee_id'])) ? ucfirst($fetch_data['employee_id']) : null; ?>">
						<span id="employee_id_error" class="text-danger p-1"></span>
					</div>
					...
					<div class="col-md-6 mt-2">
						...
						<input type="text" class="form-control form-control-lg mt-2" id="designation" name="designation" value="<?php echo (!empty($fetch_data['designation'])) ? ucfirst($fetch_data['designation']) : null; ?>">
						<span id="designation_error" class="text-danger p-1"></span>
					</div>
					...
					<div class="col-md-6 mt-2">
						...
						<input type="text" class="form-control form-control-lg mt-2" id="department" name="department" value="<?php echo (!empty($fetch_data['department'])) ? ucfirst($fetch_data['department']) : null; ?>">
						<span id="department_error" class="text-danger p-1"></span>
					</div>
					<!-- Department field end -->
					
					
					<!-- Working hours field start -->
					<div class="col-md-6 mt-2">
						<label for="working_hours">Working Hours
						<span title="Only numeric characters allowed"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#787a7b" class="bi bi-question-circle-fill" viewBox="0 0 16 16">
							<path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.496 6.033h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286a.237.237 0 0 0 .241.247zm2.325 6.443c.61 0 1.029-.394 1.029-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94 0 .533.425.927 1.01.927z"/>
						</svg></span>
						<svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" class="bi bi-star-fill star" fill="#c16e7c" viewBox="0 0 16 16">
							<path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
						...
						<input type="text" class="form-control form-control-lg mt-2" id="working_hours" name="working_hours" value="<?php echo (!empty($fetch_data['working_hour'])) ? ucfirst($fetch_data['working_hour']) : null; ?>">
						<span id="working_hours_error" class="text-danger p-1"></span>
					</div>
					<!-- Working hours field end -->
					
					
					<!-- Button field start -->
					<div class="col-md-9"></div>
					<div class="col-md-3">
						<a class="btn p-3 previous-btn text-light" href="<?php echo $base_url . '/contact_info.php?id='.$user_id.'&official_info_edit_status='.$official_info_edit_status.'&payment_info_edit_status='.$payment_info_edit_status.'&address_info_edit_status='.$address_info_edit_status; ?>">Previous</a>
						<input type="submit" class="btn p-3 next-btn text-light" id="submit2" name="submit2" value="Next">
					</div>
					<!-- Button field end -->
